...
{
    public class DiskSceneFileInfo : MonoBehaviour
    {
        ...
        public bool Exists
        {
            get
            {
                ...
                if (m_embedded)
                {
                    var meta = string.Format("{0}/{1}", m_fullpath, FN_METADATA);
            ...
            get
            {
                if (Valid)
                {
                    ...
                    else if (Directory.Exists(m_fullpath))
                    {
                        ...
                        m_creationTime = File.GetLastWriteTime(Path.Combine(m_fullpath, FN_SKETCH));
                    }
                    else
                    {
